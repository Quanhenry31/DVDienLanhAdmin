...
const UpdateProduct = (props) => {
  ...
  useEffect(() => {
    if (dataUpdate) {
      ...
      setSizes(dataUpdate?.ServiceCategories);
    }
  }, [dataUpdate]);
  ...
  const onFinish = async (values) => {
    ...
    try {
      await axios.put(`http://localhost:9000/api/servicesOrder/${dataUpdate.id}`, data);
  ...
  const handleSaveSize = async (values) => {
    try {
      if (currentSize?.id) {
        // Sửa Size
        await axios.put(`http://localhost:9000/api/servicesCategories/${currentSize.id}`, values);
  ...
  const handleDeleteSize = async (sizeId) => {
    try {
      await axios.delete(`http://localhost:9000/api/servicesCategories/${sizeId}`);
